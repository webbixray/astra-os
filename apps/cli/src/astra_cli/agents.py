...
@click.pass_context
def run_agent(ctx: click.Context, agent_id: str, input: str, file, async_mode: bool, follow: bool):
    """Run an agent"""
    console = Console()
    ...
    try:
        ...
        endpoint = f"{api_url}/api/v1/agents/{agent_id}/run"
        if async_mode:
            endpoint += "?async=true"
...
def agent_logs(ctx: click.Context, task_id: str, follow: bool, lines: int):
    ...
    try:
        ...
        response = requests.get(
            f"{ctx.obj.get('api_url', 'https://api.astra-os.io')}/api/v1/tasks/{task_id}/logs",
            headers=_get_headers(ctx.obj.get("config", {})),
            params=params,
            timeout=30,
            stream=follow,
...
def delete_agent(ctx: click.Context, agent_id: str, force: bool):
    ...
    try:
        response = requests.delete(
            f"{ctx.obj.get('api_url', 'https://api.astra-os.io')}/api/v1/agents/{agent_id}",
            headers=_get_headers(ctx.obj.get("config", {})),
            timeout=30,
        )
        response.raise_for_status()
