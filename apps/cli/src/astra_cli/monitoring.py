...
def health_check(ctx: click.Context, service: str):
    ...
    try:
        endpoint = f"{ctx.obj.get('api_url', 'https://api.astra-os.io')}/api/v1/health"
        if service:
            endpoint += f"/{service}"
...
def get_metrics(ctx: click.Context, service: str, metric: str, duration: str):
    ...
    try:
        ...
        response = requests.get(
            f"{ctx.obj.get('api_url', 'https://api.astra-os.io')}/api/v1/metrics",
            headers=headers,
            params=params,
            timeout=30,
        )
...
@click.option("--severity", "-sev", type=click.Choice(["critical", "warning", "info", "all"]), default="all")
@click.pass_context
def list_alerts(ctx: click.Context, status: str, severity: str):
    ...
    try:
        ...
        response = requests.get(
            f"{ctx.obj.get('api_url', 'https://api.astra-os.io')}/api/v1/alerts",
            headers=_get_headers(ctx.obj.get("config", {})),
            params=params,
            timeout=30,
        )
...
def show_slo(ctx: click.Context, name: str):
    ...
    try:
        ...
        response = requests.get(
            f"{ctx.obj.get('api_url', 'https://api.astra-os.io')}/api/v1/slos",
            headers=_get_headers(ctx.obj.get("config", {})),
            params=params,
            timeout=30,
        )
...
def get_trace(ctx: click.Context, trace_id: str, follow: bool):
    ...
    try:
        response = requests.get(
            f"{ctx.obj.get('api_url', 'https://api.astra-os.io')}/api/v1/traces/{trace_id}",
            headers=_get_headers(ctx.obj.get("config", {})),
            timeout=30,
        )
        response.raise_for_status()
...
def circuit_breakers(ctx: click.Context):
    ...
    try:
        response = requests.get(
            f"{ctx.obj.get('api_url', 'https://api.astra-os.io')}/api/v1/circuit-breakers",
            headers=_get_headers(ctx.obj.get("config", {})),
            timeout=30,
        )
        response.raise_for_status()
...
def dlq_status(ctx: click.Context, stream: str):
    ...
    try:
        ...
        response = requests.get(
            f"{ctx.obj.get('api_url', 'https://api.astra-os.io')}/api/v1/dlq",
            headers=_get_headers(ctx.obj.get("config", {})),
            params=params,
            timeout=30,
        )
...
# Export the group
monitoring_cli = monitoring_group
