...
def list_schemas(ctx: click.Context, output: str):
    ...
    try:
        ...
        response = requests.get(
            f"{ctx.obj['api_url']}/api/v1/schemas",
            headers=headers,
            timeout=30,
        )
...
def get_schema(ctx: click.Context, schema_name: str, output: str):
    ...
    try:
        headers = _get_headers(ctx.obj["config"])
        response = requests.get(
            f"{ctx.obj['api_url']}/api/v1/schemas/{schema_name}",
            headers=_get_headers(ctx.obj["config"]),
            timeout=30,
        )
        response.raise_for_status()
...
def validate_schema(ctx: click.Context, schema_name: str, file, data: str):
    ...
    try:
        ...
        response = requests.post(
            f"{ctx.obj['api_url']}/api/v1/schemas/{schema_name}/validate",
            headers=_get_headers(ctx.obj["config"]),
...
def get_example(ctx: click.Context, schema_name: str, output: str):
    ...
    try:
        headers = _get_headers(ctx.obj["config"])
        response = requests.get(
            f"{ctx.obj['api_url']}/api/v1/schemas/{schema_name}/example",
            headers=headers,
            timeout=30,
        )
        response.raise_for_status()
