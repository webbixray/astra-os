...
def list_workflows(ctx: click.Context, status: str, org: str, limit: int, output: str):
    ...
    try:
        ...
        response = requests.get(
            f"{ctx.obj['api_url']}/api/v1/workflows",
            headers=_get_headers(ctx.obj["config"]),
            params={"limit": limit, "status": status, "organization_id": org},
            timeout=30,
        )
...
def get_workflow(ctx: click.Context, workflow_id: str, output: str):
    ...
    try:
        response = requests.get(
            f"{ctx.obj['api_url']}/api/v1/workflows/{workflow_id}",
            headers=_get_headers(ctx.obj["config"]),
            timeout=30,
        )
        response.raise_for_status()
...
@click.option("--follow", "-f", is_flag=True, help="Follow execution")
@click.pass_context
def run_workflow(
    ...
    try:
        headers = _get_headers(ctx.obj["config"])
        endpoint = f"{ctx.obj['api_url']}/api/v1/workflows/{workflow_id}/run"
        if async_mode:
            endpoint += "?async=true"
...
def workflow_executions(ctx: click.Context, workflow_id: str, status: str, limit: int):
    ...
    try:
        ...
        response = requests.get(
            f"{ctx.obj['api_url']}/api/v1/workflows/{workflow_id}/executions",
            headers=_get_headers(ctx.obj["config"]),
            params=params,
            timeout=30,
        )
