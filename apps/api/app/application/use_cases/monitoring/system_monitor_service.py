...
class SystemMonitorService:
    ...
    async def log_action(
        self, org_id: UUID, user_id: UUID, action: str,
    ...
    async def get_audit_logs(
        ...
        entries = await self.audit_repo.find_by_org(
            org_id, action, resource_type, user_id, limit, offset,
        )
    ...
    async def create_job(
        self, org_id: UUID, job_type: str,
        payload: dict | None = None, max_retries: int = 3,
    ) -> JobRecord:
        job = JobRecord(
            organization_id=org_id, job_type=job_type,
            payload=payload or {}, max_retries=max_retries,
        )
    ...
    async def get_jobs(
        ...
        items = [
            {
                "id": str(j.id), "job_type": j.job_type,
    ...
    async def update_job_status(
        ...
        result: dict | None = None, error_message: str = "",
    ) -> JobRecord:
    ...
    async def record_api_call(
        ...
        ip_address: str = "", response_time_ms: float = 0.0,
    ) -> ApiUsageRecord:
