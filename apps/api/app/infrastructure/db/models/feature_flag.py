...
class FeatureFlagModel(Base):
    ...
    def to_domain(self) -> FeatureFlag:
        return FeatureFlag(
            ...
            feature_key=self.feature_key, enabled=self.enabled,
    ...
    def from_domain(cls, ff: FeatureFlag) -> "FeatureFlagModel":
        return cls(
            id=ff.id, organization_id=ff.organization_id,
            feature_key=ff.feature_key, enabled=ff.enabled,
            config=ff.config,
            created_at=ff.created_at, updated_at=ff.updated_at,
        )
