...
class ContentRecommendationModel(Base):
    ...
    def from_domain(cls, r: ContentRecommendation) -> "ContentRecommendationModel":
        return cls(
            ...
            recommendation_metadata=r.metadata,
            applied=r.applied,
            created_at=r.created_at,
        )
...
class AutomationRuleModel(Base):
    ...
    def to_domain(self) -> AutomationRule:
        return AutomationRule(
            ...
            enabled=self.enabled,
            last_evaluated_at=self.last_evaluated_at.replace(tzinfo=None)
            if self.last_evaluated_at
            else None,
            last_triggered_at=self.last_triggered_at.replace(tzinfo=None)
