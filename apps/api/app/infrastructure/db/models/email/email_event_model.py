...
class EmailEventModel(Base):
    ...
    def to_domain(self) -> EmailEvent:
        return EmailEvent(
            id=self.id, campaign_id=self.campaign_id, event_type=self.event_type,
            recipient_email=self.recipient_email, metadata=dict(self.event_metadata) if self.event_metadata else {},
            occurred_at=self.occurred_at.replace(tzinfo=None), created_at=self.created_at.replace(tzinfo=None),
        )

    @classmethod
    def from_domain(cls, e: EmailEvent) -> "EmailEventModel":
        return cls(
            id=e.id, campaign_id=e.campaign_id, event_type=e.event_type,
            recipient_email=e.recipient_email, event_metadata=e.metadata,
