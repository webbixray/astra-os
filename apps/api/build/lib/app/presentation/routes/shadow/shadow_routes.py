...
from app.infrastructure.db.repositories.shadow_repository import (
    LiftMeasurementRepository,
    ShadowDecisionRepository,
    ShadowEventRepository,
...
def get_shadow_session_service(db: AsyncSession = Depends(get_db)) -> ShadowSessionService:
    ...
    session_repo = ShadowSessionRepository(db)
    decision_repo = ShadowDecisionRepository(db)
    event_repo = ShadowEventRepository(db)
...
def get_lift_measurement_service(db: AsyncSession = Depends(get_db)) -> LiftMeasurementService:
    ...
    session_repo = ShadowSessionRepository(db)
    decision_repo = ShadowDecisionRepository(db)
    measurement_repo = LiftMeasurementRepository(db)
...
async def get_session_events(
    ...
    from app.domain.services.shadow_mode import ShadowDecisionService
    from app.infrastructure.db.repositories.shadow_repository import ShadowEventRepository

    event_repo = ShadowEventRepository(db)
    service = ShadowDecisionService(event_repo)
