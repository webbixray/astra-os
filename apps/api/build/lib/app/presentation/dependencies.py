...
async def get_redis() -> RedisCache:
    """Get the Redis cache instance from app state."""
    global _redis_cache
    if _redis_cache is None:
        _redis_cache = RedisCache(RedisConfig())
...
async def require_organization_id(request: Request) -> UUID:
    """Get organization ID from request state (set by TenantResolutionMiddleware)."""
