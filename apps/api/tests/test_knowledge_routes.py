...
class TestRAGRoutes:
    def test_search_empty(self, client, org_id):
        ...
        resp = c.post("/knowledge/rag/search", json={
    ...
    def test_search_with_results(self, client, org_id):
        ...
        mock_pipeline.search = AsyncMock(return_value=[
            SearchResult(
                ...
                name="Summer Sale", description="A great campaign",
                score=0.9, source="vector", properties={},
            ),
        ])
    ...
    def test_context_assembly(self, client, org_id):
        c, mock_pipeline, _, _ = client
        mock_pipeline.assemble_context = AsyncMock(return_value=RAGContext(
            query="test", results=[], context_text="No context",
            source_node_ids=[], organization_id=org_id,
        ))

        resp = c.post("/knowledge/rag/context", json={
            "organization_id": org_id,
            "query": "test",
        })
        assert resp.status_code == 200
    ...
    def test_ingest_brand_guidelines(self, client, org_id):
        c, mock_pipeline, _, _ = client
        mock_pipeline.ingest_brand_guidelines = AsyncMock(return_value=IngestionResult(
            nodes_created=1, memories_created=1, processing_time_ms=42.5,
        ))

        resp = c.post("/knowledge/rag/ingest/brand-guidelines", json={
            "organization_id": org_id,
    ...
    def test_ingest_campaign_data(self, client, org_id):
        c, mock_pipeline, _, _ = client
        mock_pipeline.ingest_campaign_data = AsyncMock(return_value=IngestionResult(
            nodes_created=1, processing_time_ms=10.0,
        ))

        resp = c.post("/knowledge/rag/ingest/campaign-data", json={
            "organization_id": org_id,
...
class TestOptimizationRoutes:
    def test_budget_optimization(self, client, org_id):
        ...
        mock_optimizer.optimize_budget = AsyncMock(return_value=[
            BudgetAllocation(
                campaign_id="1", campaign_name="A",
                current_daily_budget=100, suggested_daily_budget=120,
                rationale="Strong ROAS", confidence=0.8,
            ),
        ])

        resp = c.post("/knowledge/optimization/budget", json={
            "organization_id": org_id,
            "campaigns": [{"id": "1", "name": "A", "daily_budget": 100, "roas": 5.0, "impressions": 10000, "clicks": 500, "conversions": 80, "spend": 100}],
        })
        assert resp.status_code == 200
        data = resp.json()
    ...
    def test_creative_fatigue(self, client, org_id):
        c, _, mock_optimizer, _ = client
        mock_optimizer.detect_creative_fatigue = AsyncMock(return_value=[
            CreativeFatigueResult(
        ...
        resp = c.post("/knowledge/optimization/creative-fatigue", json={
            "organization_id": org_id,
            "creatives": [{"id": "c1", "name": "Ad A", "ctr_history": [0.05, 0.02], "current_ctr": 0.02, "peak_ctr": 0.05}],
        })
        assert resp.status_code == 200
        data = resp.json()
    ...
    def test_audience_expansion(self, client, org_id):
        ...
        resp = c.post("/knowledge/optimization/audience-expansion", json={
            "organization_id": org_id,
    ...
    def test_suggestions(self, client, org_id):
        ...
        resp = c.post("/knowledge/optimization/suggestions", json={
            "organization_id": org_id,
...
class TestCrossCampaignRoutes:
    def test_mine_patterns(self, client, org_id):
        ...
        mock_learner.mine_patterns = AsyncMock(return_value=[
            CampaignPattern(
                pattern_id="p1", pattern_type=PatternType.AUDIENCE_PATTERN,
                title="Shared audience", description="desc",
                campaign_ids=["c1", "c2"], strength=0.8,
                sample_size=2, confidence=0.75,
            ),
        ])

        resp = c.post("/knowledge/patterns/mine", json={
            "organization_id": org_id,
        })
        assert resp.status_code == 200
        data = resp.json()
    ...
    def test_transfer_suggestions(self, client, org_id):
        ...
        resp = c.post("/knowledge/patterns/transfer", json={
            "organization_id": org_id,
    ...
    def test_learning_insights(self, client, org_id):
        c, _, _, mock_learner = client
        mock_learner.get_learning_insights = AsyncMock(return_value=[
            LearningInsight(
                insight_id="i1", title="High-performing pattern",
                description="desc", supporting_campaigns=["c1"],
                insight_type="audience_pattern", priority="high",
                recommended_actions=["Review"],
            ),
        ])

        resp = c.post("/knowledge/patterns/insights", json={
            "organization_id": org_id,
        })
        assert resp.status_code == 200
        data = resp.json()
