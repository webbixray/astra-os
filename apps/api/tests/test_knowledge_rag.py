...
def mock_ms():
    """Mock MemoryService."""
    ms = MagicMock()
    ms.remember = AsyncMock(
        return_value={"id": str(uuid4()), "key": "test", "type": "fact", "importance": "medium"}
    )
...
class TestSearchResult:
    ...
    def test_default_values(self):
        r = SearchResult(
            node_id="1", node_type="campaign", name="T", description="d", score=0.5, source="x"
        )
        assert r.properties == {}
...
class TestRAGContext:
    def test_result_count(self):
        results = [
            SearchResult(
                node_id=str(i), node_type="x", name="n", description="d", score=0.5, source="v"
            )
            for i in range(5)
        ]
    ...
    def test_high_relevance_count(self):
        results = [
            SearchResult(
                node_id="1", node_type="x", name="n", description="d", score=0.9, source="v"
            ),
            SearchResult(
                node_id="2", node_type="x", name="n", description="d", score=0.5, source="v"
            ),
        ]
        ctx = RAGContext(query="test", results=results, context_text="ctx", source_node_ids=[])
...
class TestRagPipelineContext:
    ...
    async def test_assemble_context_includes_memories(
        self, pipeline, org_id, user_id, mock_ks, mock_ms
    ):
        mock_ks.search = AsyncMock(return_value=[])
        mock_ms.recall = AsyncMock(
            return_value=[
                {"key": "preference", "value": "likes dark mode", "importance": "high"},
            ]
        )
        ctx = await pipeline.assemble_context(
            "test", org_id, user_id=user_id, include_memories=True
        )
    ...
    async def test_assemble_context_brand_guidelines(self, pipeline, org_id, mock_ks):
        ...
        ctx = await pipeline.assemble_context(
            "test", org_id, include_brand_guidelines=True, include_memories=False
        )
        assert ctx.brand_guidelines == "Use blue color"

    async def test_assemble_context_no_brand_guidelines_when_disabled(
        self, pipeline, org_id, mock_ks
    ):
        mock_ks.search = AsyncMock(return_value=[])
        ctx = await pipeline.assemble_context(
            "test", org_id, include_brand_guidelines=False, include_memories=False
        )
    ...
    async def test_assemble_context_agent_id(self, pipeline, org_id, mock_ks):
        mock_ks.search = AsyncMock(return_value=[])
        ctx = await pipeline.assemble_context(
            "test", org_id, agent_id="ceo_agent", include_memories=False
        )
...
class TestRagPipelineIngestion:
    ...
    async def test_ingest_brand_guidelines_with_user(
        self, pipeline, org_id, user_id, mock_ks, mock_ms
    ):
        result = await pipeline.ingest_brand_guidelines(
            org_id,
    ...
    async def test_ingest_campaign_data_with_user(
        self, pipeline, org_id, user_id, mock_ks, mock_ms
    ):
        cid = uuid4()
        result = await pipeline.ingest_campaign_data(
...
class TestMergeResults:
    def test_merge_deduplicates(self, pipeline):
        nid = str(uuid4())
        v = SearchResult(
            node_id=nid, node_type="x", name="n", description="d", score=0.8, source="vector"
        )
        k = SearchResult(
            node_id=nid, node_type="x", name="n", description="d", score=0.6, source="keyword"
        )
    ...
    def test_merge_keeps_unique(self, pipeline):
        v = SearchResult(
            node_id="1", node_type="x", name="n", description="d", score=0.8, source="vector"
        )
        k = SearchResult(
            node_id="2", node_type="x", name="n", description="d", score=0.6, source="keyword"
        )
        merged = pipeline._merge_results([v], [k])
        assert len(merged) == 2
