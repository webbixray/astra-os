...
import { KpiCardWidget, ChartWidget, PieChartWidget, DataTableWidget, TrendWidget } from '@/components/widgets';
import {
  useDashboards,
  useDashboardDetail,
  useCreateDashboard,
  ...
  useAnomalies,
  usePredictions,
} from '@/features/analytics/api/useDashboards';
import { useOrg } from '@/lib/org';
import { ErrorBoundary } from '@/components/ui/error-boundary';

const WIDGET_TYPE_OPTIONS = [
  { type: 'kpi_card', label: 'KPI Card', icon: BarChart3, defaultW: 1, defaultH: 1 },
  { type: 'line_chart', label: 'Line Chart', icon: LineChart, defaultW: 2, defaultH: 2 },
  { type: 'bar_chart', label: 'Bar Chart', icon: BarChart3, defaultW: 2, defaultH: 2 },
  { type: 'pie_chart', label: 'Pie Chart', icon: PieChart, defaultW: 2, defaultH: 2 },
  { type: 'data_table', label: 'Data Table', icon: Table, defaultW: 3, defaultH: 2 },
  { type: 'trend_indicator', label: 'Trend', icon: TrendingUp, defaultW: 1, defaultH: 1 },
];

const METRIC_OPTIONS = [
  { value: 'campaigns_total', label: 'Total Campaigns' },
  { value: 'campaigns_active', label: 'Active Campaigns' },
  { value: 'ad_spend', label: 'Ad Spend' },
  { value: 'ad_impressions', label: 'Impressions' },
  { value: 'ad_clicks', label: 'Clicks' },
  { value: 'ad_conversions', label: 'Conversions' },
  { value: 'ad_ctr', label: 'CTR' },
  { value: 'content_published', label: 'Published Content' },
...
export default function DashboardPage() {
  ...
  const [selectedDashId, setSelectedDashId] = useState<string | undefined>(undefined);
  ...
  const handleCreate = () => {
    ...
    createDash.mutate(
      { organization_id: orgId, name: newName.trim() },
      {
        onSuccess: (d) => {
  ...
  if (widgetData) {
    widgetData.forEach((wd) => {
      const key = wd.config?.metric as string;
      if (key) metricValues.set(key, wd.data.value);
    });
  }
        ...
        <div className="hidden md:block w-56 shrink-0 border-r overflow-y-auto p-3">
          ...
          {showCreate && (
            <div className="mt-3 space-y-2 rounded-md border p-3">
              <label htmlFor="dash-name" className="text-xs font-medium text-muted-foreground">Dashboard name</label>
              <Input
                id="dash-name"
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60" role="dialog" aria-modal="true" aria-label="Add Widget" onClick={() => setShowWidgetPicker(false)}>
          <div className="w-full max-w-lg rounded-lg border bg-card p-6 shadow-lg" onClick={(e) => e.stopPropagation()}>
            <div className="mb-4 flex items-center justify-between">
              ...
              <button aria-label="Close widget picker" onClick={() => setShowWidgetPicker(false)}><X className="h-4 w-4" /></button>
            </div>
            <div className="grid grid-cols-3 gap-3">
              {WIDGET_TYPE_OPTIONS.map((opt) => (
                ...
                >
                  ...
                  <span className="text-[10px] text-muted-foreground">{opt.defaultW}×{opt.defaultH}</span>
                </button>
              ))}
            </div>
          </div>
